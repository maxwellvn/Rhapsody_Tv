 ...
 * 
 * NOTE: Push notifications require a development build (native APK/IPA).
 * They do NOT work in Expo Go as of SDK 53.
 */
...
// Dynamically import expo-notifications to handle Expo Go gracefully
...
// Try to initialize notifications (will fail gracefully in Expo Go)
try {
  if (!isExpoGo) {
    ...
    console.log('[PushNotification] Running in Expo Go - push notifications disabled');
  }
} catch (error) {
  console.log('[PushNotification] Native modules not available');
}
...
class PushNotificationService {
  ...
  async initialize(): Promise<boolean> {
    // Skip if in Expo Go or modules not available
    if (isExpoGo || !Notifications || !Device) {
      console.log('[PushNotification] Skipped - running in Expo Go or native modules unavailable');
      return false;
    }
   ...
   */
  isAvailable(): boolean {
    return !isExpoGo && !!Device?.isDevice && this.initialized;
  }
