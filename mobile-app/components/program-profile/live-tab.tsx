...
export function LiveTab({ programId, programName }: LiveTabProps) {
  const router = useRouter();

  const { data, isLoading, error } = useQuery({
    queryKey: ['programLivestreams', programId],
    queryFn: async () => {
      const response = await livestreamService.getByProgram(programId);
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>
          {programName} doesn't have any active or upcoming livestreams right now.
        </Text>
      </View>
    );
  }
